...
class DETR(nn.Module):
    ...
    def get_rpn(self,features, image_tensor, b_size):
        images = ImageList(image_tensor, 
                        [(image_tensor.shape[2], image_tensor.shape[3]) for _ in range(b_size)])
        ...
        rois = torch.zeros((b_size * num_proposals, 5))
        batch_indices = torch.arange(b_size).unsqueeze(1).expand(-1, num_proposals).flatten()

        xmin = proposals[..., 0].flatten()
        ymin = proposals[..., 1].flatten()
        xmax = proposals[..., 2].flatten()
        ymax = proposals[..., 3].flatten()

        rois[:, 0] = batch_indices
        rois[:, 1] = xmin
        rois[:, 2] = ymin
        rois[:, 3] = xmax
        rois[:, 4] = ymax
    ...
    def forward(self, samples: NestedTensor):
        ...
        """
        #######################################################################
        b_size = samples.tensors.shape[0]
        ########################################################################
        ...
        hs = self.transformer(self.input_proj(src), mask, qeury_embed_new, pos[-1])[0]
